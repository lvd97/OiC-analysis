...
def inaccurate_to_na(df, p):
    """ Sets all values in dataframa lower than the p threshold to NA
...
def cummulative_time(col):
    ...
    -------
    series
        A column that shows the commulative time that a value was True
    """
    col = col.cumsum()
    col = col/12.5
    return col
...
def write_video(animal, video, outpath, df, fps, coords_object1_scaled, coords_object2_scaled, coords_object1_og, coords_object2_og):
    ...
    outpath:path   
        The path the video should be saved 
    df: pd.Dataframe
    ...
    df['cum_time_object1'] = cummulative_time(df['in_object1'])
    df['cum_time_object2'] = cummulative_time(df['in_object2'])

    frame_width = int(video.get(3))
    frame_height = int(video.get(4))
    res = (frame_width, frame_height)

    # to force video output to be of type .mp4
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(outpath+animal + '_DLC.mp4', fourcc, fps, res)
    ...
    while(ret):
        ...
        cv2.putText(frame, text_object2, (175, 30), font,
...
def get_info(animal, p, offset, df, fps, coords_object1, coords_object2, video, outpath, make_video=True):
    ...
    poly_object1_scaled, coords_object1_scaled = create_polygon(
        coords_object1.X.copy(), coords_object1.Y.copy(), offset)  # .copy() to prevent overwriting origional polygon
